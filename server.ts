...
数値を1つだけ。水耕栽培や土耕栽培の測定データに自動入力するための「摂氏の現在（最高または平均）気温」を半角数値（小数点1位以下は四捨五入して整数、または1位まで、例: 22 または 21.5）だけで出力してください。単位、テキスト、前置きは一切不要です。（出力形式の例： 21.5 または 18）`;

      const aiResponse = await geminiClient.models.generateContent({
        model: "gemini-3.1-flash-lite",
        contents: prompt,
        config: {
          systemInstruction: "あなたは現在のリアルタイムの現地気温を計測して数値だけで回答するシステムです。余計な文字列（「摂氏」「度」「°C」など）を絶対に含めないで、数値「23.1」や「18」のように出力します。",
          tools: [{ googleSearch: {} }]
        }
      });
